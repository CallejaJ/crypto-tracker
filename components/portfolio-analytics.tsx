...
import { TrendingUp, TrendingDown, LucidePieChart, BarChart3, Activity, Shield } from "lucide-react"
import {
  LineChart,
  Line,
  XAxis,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {performanceMetrics.map((metric) => (
          <Card key={metric.period} className="bg-card">
            ...
            <CardContent>
              <div className="text-2xl font-heading font-black text-card-foreground">{metric.value}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card">
          <CardHeader>
            <CardTitle className="text-xl font-heading font-bold text-card-foreground">Performance Ranking</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {topPerformers.map((performer, index) => (
                <div key={performer.coin} className="flex items-center justify-between p-3 rounded-lg bg-muted/20">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center">
                      <span className="text-primary-foreground font-bold text-xs">#{index + 1}</span>
                    </div>
                    <div>
                      <span className="font-heading font-bold text-card-foreground">{performer.coin}</span>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className={`text-sm font-medium ${performer.change > 0 ? "text-green-500" : "text-red-500"}`}>
                      {performer.change > 0 ? "+" : ""}
                      {performer.change}%
                    </span>
                  </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {riskMetrics.map((risk) => (
              <div key={risk.metric} className="p-4 rounded-lg bg-muted/20 border border-border">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-muted-foreground">{risk.metric}</span>
                  <Badge className={`${getRiskColor(risk.status)} text-white text-xs`}>{risk.status}</Badge>
                </div>
                <div className="text-lg font-heading font-bold text-card-foreground">{risk.value}</div>
              </div>
            ))}
          </div>
        </CardContent>
